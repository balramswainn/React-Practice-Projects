...
exports.getEditHome = (req, res, next) => {  // host homes me edit pe click karega toh ye open hoga
 ...
 Home.findById(homeId).then(([homes]) => {        //findById se hume woh id jisme  woh object hai mil jaega
   const home = homes[0];  //bcz ye hume [{id:}] array me dega but hume object chahiye isliye homes[0]
...
exports.getHostHomes = (req, res, next) => {  //jisne bhi form bhara hai muje woh data chahiye so me fetchAll se data lelunga 
  Home.fetchAll().then(([registeredHomes]) =>{          // fetchAll ->model me hai jo hume homes.json ka data parse karke dega 
...
exports.postAddHome = (req, res, next) => {              //post req hai toh req me iska pas form ka data hoga isliye req.body
 ...
 const home = new Home(houseName, price, location, rating, photoUrl, description); //id nhi rha toh bhi chalega js me function kk arguement sare match ho jaruri nhi so id undefined chala jaega
